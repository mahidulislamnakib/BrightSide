...
export const seedRouter = createRouter({
  run: publicQuery.query(async () => {
    const db = getDb();
    const count = await getArticleCount();
    if (count > 0) {
      return { seeded: false, message: "Database already contains data", count };
    }

    await db.insert(sources).values(SEED_SOURCES);
    await db.insert(categories).values(SEED_CATEGORIES);
    await db.insert(articles).values(SEED_ARTICLES);

    return { seeded: true, message: "Database seeded successfully", articles: SEED_ARTICLES.length };
  }),
});
